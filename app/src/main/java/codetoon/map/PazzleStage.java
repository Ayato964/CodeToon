...
public class PazzleStage extends Map{
    ...
    public PazzleStage(Rule r){
        ...
        firstPath = 0;
        if(r.isSettingMemoryPassword)
             firstPath = new Random().nextInt(1000, 10000);
        if(!r.isShowingOpponentMemory) {

            field = new Field(5, 15, 130, 83);
            field.setMemoryCapability(MEMORY_W, MEMORY_H,firstPath);
        }else{
    ...
    public void setup(Graphics h) {
        if(!CodeToon.DEBUG) {
            if (!rule.isShowingOpponentMemory) {
            ...
            if(rule.isSettingMemoryPassword)
                Message.addMessage(new String[]{new StringBuilder().append(firstPath).toString()}, "stage.memory.pass");
        }
        if(CodeToon.DEBUG){
    ...
    private class Field{
      ...
      public void setMemoryCapability(int mw, int mh, int pass){
          this.mw = mw;
          this.mh = mh;
        Memories.setInstance(mw, mh, x, y, w, h, pass);
      }
      public void display(Graphics g){
          if(!enemyMode) {
