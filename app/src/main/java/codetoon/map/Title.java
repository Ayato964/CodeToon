...
public class Title extends Map{
    ...
    box = new ContainerBox(60, 40, 70, 40, new ContainerData<Box, Integer>(){
      ...
      public void action(int i){
        switch(i){
          case 0:
          Server.isHost = true; Main.getInstance().run(new CreateSection());
             /* Main.getInstance().run(new PazzleStage(5)); Server.server.setUpServer();*/ break;
          case 1: System.out.println("途中からゲームを始める。");Server.isHost = false; Main.getInstance().run(new PazzleStage(5)); Server.server.connect("192.168.11.13"); break;
